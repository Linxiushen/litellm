...
- ``GatewayRequestSink`` receives every classified request with its status and
  ...
  Not license-gated (see litellm.proxy.db.gateway_request_tracking).
...
class GatewayRequestSink(Protocol):
    ...
    Distinct from BillingRecorder: this is not license-gated and not restricted
    to 2xx, so the admin UI reports gateway traffic on any deployment.
    """
...
class BillableRequestMetricsMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if sink is not None:
            try:
                sink.record(category=category, route=route, status_code=status_code, model_id=model_id)
