...
_ROWS_ADAPTER = TypeAdapter(list[_AggregateRow])
...
def _fold_by_date(rows: list[_AggregateRow]) -> list[GatewayRequestDailyEntry]:
...
def _fold_by_route(rows: list[_AggregateRow]) -> list[GatewayRequestBreakdownEntry]:
    pairs = sorted({(row.category, row.route) for row in rows})
    entries = [
        GatewayRequestBreakdownEntry(
            category=category,
            route=route,
    ...
    return sorted(entries, key=lambda entry: entry.successful_requests, reverse=True)
...
async def get_gateway_daily_activity(
    ...
    raw_rows = await prisma_client.db.query_raw(  # pyright: ignore[reportAny]  # untyped prisma client
        _AGGREGATE_SQL,
    ...
    rows = _ROWS_ADAPTER.validate_python(raw_rows or [])
